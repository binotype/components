...
export class Parser<Node> {
	constructor(private convert: (content: string | dom.Block[] | undefined) => Promise<binotype.Content<Node>>) {}
	async import(type: "block", node: dom.Block.Section): Promise<binotype.Block<Node> & { id: string }>
	async import(type: "page", node: dom.Document): Promise<binotype.Page<Node> & { id: string }>
	async import(
		type: "block" | "page",
		node: dom.Block.Section | dom.File
	): Promise<(binotype.Block<Node> & { id: string }) | (binotype.Page<Node> & { id: string })> {
		const properties = {
		...
		return {
			...
			meta: meta as binotype.Meta,
			mode: binotype.Mode.parse(dom.Variables.parse("string", variables, "mode")),
	...
	async parse(content: string, name?: string): Promise<(binotype.Page<Node> & { id: string }) | undefined> {
	...
	async open(path: string): Promise<(binotype.Page<Node> & { id: string }) | undefined> {
